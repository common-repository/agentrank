<?php
/**
 *AgentRank Virtual Page Class
 *Since Version 1.0 
*/

class Create_Agentrank_Page
{
	/**
	 ...
	 * Class constructor
	 */
	function Create_Agentrank_Page($slug,$title,$con)
	{
		/**
	...
	{	/**
		...
		if($con == "sale"){
		//include sale page if questions api invoked by http get
        $sale = include (WP_PLUGIN_DIR . '/agentrank/sale.php');
		return $sale;
		}elseif ($con == 'sales'){
		$sales = include (WP_PLUGIN_DIR . '/agentrank/sales.php');
		return $sales;
		}elseif ($con == 'review'){
		$review = include (WP_PLUGIN_DIR . '/agentrank/review.php');
		return $review;
		}elseif ($con == 'reviews'){
		$reviews = include (WP_PLUGIN_DIR . '/agentrank/reviews.php');
		return $reviews;
		}elseif ($con == 'forecast'){
		$forecast = include (WP_PLUGIN_DIR . '/agentrank/forecast.php');
		return $forecast;
		}elseif ($con == 'forecasts'){
		$forecasts = include (WP_PLUGIN_DIR . '/agentrank/forecasts.php');
		return $forecasts;
		}elseif ($con == 'contact'){
		$contact = include (WP_PLUGIN_DIR . '/agentrank/contact.php');
		return $contact;
		};
	}
 ...
 */
new Create_Agentrank_Page('agentrank/sale','','sale');
new Create_Agentrank_Page('agentrank/sales','','sales');
new Create_Agentrank_Page('agentrank/review','','review');
new Create_Agentrank_Page('agentrank/reviews','','reviews');
new Create_Agentrank_Page('agentrank/forecast','','forecast');
new Create_Agentrank_Page('agentrank/forecasts','','forecasts');
new Create_Agentrank_Page('agentrank/contact','','contact');
?>